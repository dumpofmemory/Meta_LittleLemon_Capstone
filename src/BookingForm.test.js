...
describe('BookingForm Component', () => {
  ...
  test('Renders the BookingForm heading', () => {
    renderBookingForm();
    expect(screen.getByText('Please fill out the form fields below')).toBeInTheDocument();
  });
  ...
  test('shows validation errors for invalid inputs', async () => {
    renderBookingForm();
    await userEvent.type(screen.getByLabelText('Number of Guests:'), '11');
  ...
  test('BookingForm Component submits form successfully with valid inputs', async () => {
    ...
    // Verify no failure alert is called
    expect(window.alert).not.toHaveBeenCalled();

    window.alert.mockRestore();
  });

  test('BookingForm Component shows failure alert when submission fails', async () => {
    const initialTimes = ['10:00', '11:00', '12:00', '18:00'];
    const today = "2025-04-06";
    const mockAvailableTimes = jest.fn((date) => initialTimes);
    const mockBookTime = jest.fn();
    const mockSetSelectedDate = jest.fn();
    const mockUpdateTimes = jest.fn();
    const mockSubmitForm = jest.fn(() => false); // Simulate failure
    ...
    // Assert submission and failure alert
    expect(mockSubmitForm).toHaveBeenCalledWith({
      date: '2025-04-06',
      time: '18:00',
      guests: 5,
    ...
    expect(window.alert).toHaveBeenCalledWith('Failed to submit reservation. Please try again.');
...
describe('Bookings Initialization', () => {
  test('initializeBookings returns an empty object', () => {
    ...
    expect(initializeBookings()).toEqual({});
  });
});
